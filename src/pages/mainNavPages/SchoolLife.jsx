        ...
        <div className="max-w-6xl mx-auto px-4 lg:px-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 lg:gap-8">
            {facilities.map((facility, index) => (
              <div
                key={index}
                className="bg-white rounded-lg shadow-lg hover-lift overflow-hidden"
              >
                <img
                  src={facility.image}
                  alt={facility.name}
        ...
        <div className="max-w-6xl mx-auto px-4 lg:px-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
            {activities.map((activity, index) => (
              <div
                key={index}
                className="text-center p-6 bg-gray-50 rounded-lg shadow-lg hover-lift"
              >
                <div className="w-16 h-16 bg-accent rounded-full flex items-center justify-center mx-auto mb-4">
                  <span className="text-white text-2xl">
                    {index === 0 && '📖'} {/* Knowledge */}
                    {index === 1 && '🤝'} {/* Community */}
                    {index === 2 && '📿'} {/* Spiritual growth */}
                    {index === 3 && '🕌'} {/* Worship & service */}
                  </span>
                </div>
        ...
        <div className="max-w-6xl mx-auto px-4 lg:px-8">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            {clubs.map((club, index) => (
              <div
                key={index}
                className="bg-white p-4 rounded-lg shadow hover-lift text-center border-l-4 border-accent"
              >
        ...
        <div className="max-w-6xl mx-auto px-4 lg:px-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {sports.map((sport, index) => (
              <div
                key={index}
                className="bg-gray-50 p-6 rounded-lg shadow-lg hover-lift"
              >
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
              <div>
                ...
                <div className="text-sm text-white/80">
                  {t('schoolLife.stats.studentClubs')}
                </div>
              </div>
              <div>
                <div className="text-3xl font-bold">100+</div>
                <div className="text-sm text-white/80">
                  {t('schoolLife.stats.annualEvents')}
                </div>
              </div>
              <div>
                <div className="text-3xl font-bold">12</div>
                <div className="text-sm text-white/80">
                  {t('schoolLife.stats.sportsTeams')}
                </div>
              </div>
            </div>
